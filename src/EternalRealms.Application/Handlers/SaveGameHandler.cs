...
{
    public sealed class SaveGameHandler
    {
        ...
        public void Handle(SaveGameCommand command)
        {
            var character = _characterRepository.LoadActive() ?? throw new InvalidOperationException("No active character to save.");
            var saveGame = new SaveGameDto
            {
                Character = MapCharacter(character),
                Quests = _questRepository.GetAvailable().Select(MapQuest).ToList()
            };

            _saveGameRepository.Save(saveGame, command.Path);
        }

        private static CharacterDto MapCharacter(Character character)
        {
